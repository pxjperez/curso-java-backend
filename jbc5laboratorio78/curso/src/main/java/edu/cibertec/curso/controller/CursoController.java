...
public class CursoController {
    ...
    public List<CursoEntity> listarCursos() {
        try{
            log.info("Instancia ejecutada en el puerto: " + puerto);
            List<CursoEntity> listaCurso = cursoService.listarCursos();
            listaCurso.forEach(c->c.add(linkTo(methodOn(CursoController.class).obtenerCurso(c.getIdCurso())).withRel("curso")));
            return listaCurso;
        }catch(Exception ex){
    ...
    public ResponseEntity<CursoEntity> obtenerCurso(@PathVariable("idCurso") Integer idCurso) {
        try{
            CursoEntity repuesta = cursoService.obtenerCurso(idCurso);
            repuesta.add(linkTo(methodOn(CursoController.class).obtenerCurso(idCurso)).withSelfRel());
